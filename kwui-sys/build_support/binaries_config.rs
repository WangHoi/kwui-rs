...
impl BinariesConfiguration {
    ...
    pub fn commit_to_cargo(&self) {
        let mut out_lib_dir = self.output_directory.clone();
        out_lib_dir.push("lib");
        cargo::add_link_search(out_lib_dir.to_str().unwrap());
    ...
    fn copy_libs_and_additional_files(
        ...
        for lib in self.built_libraries(copy_bindings) {
            let filenames = &target.library_to_filenames(lib);
        ...
        fn copy(file_name: &Path, from_dir: &Path, to_dir: &Path) -> io::Result<()> {
            ...
            fs::create_dir_all(to_path.parent().unwrap()).map_err(|e| {
                eprintln!(
                    "COPY OPERATION FAILED: create parent dir '{}': {}",
                    to_path.display(),
                    e
                );
                e
