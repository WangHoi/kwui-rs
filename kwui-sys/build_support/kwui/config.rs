...
impl BuildConfiguration {
    pub fn from_features(features: features::Features, kwui_debug: bool) -> Self {
...
pub struct FinalBuildConfiguration {
    ...
    pub kwui_source_dir: PathBuf,

    /// Arguments passed to cmake.
    pub cmake_args: Vec<(String, String)>,
...
impl FinalBuildConfiguration {
    pub fn from_build_configuration(
        ...
        let mut builder = CMakeArgsBuilder::new(&build.target);

        let cmake_args = {
            builder
                .arg("is_official_build", yes_if(!build.kwui_debug))
                .arg("is_debug", yes_if(build.kwui_debug))
        ...
        FinalBuildConfiguration {
            kwui_source_dir: kwui_source_dir.into(),
            cmake_args,
...
pub fn build(
    ...
    _config: &binaries_config::BinariesConfiguration,
    _ninja_command: Option<PathBuf>,
    _cmake_command: Option<PathBuf>,
    _offline: bool,
) {
    if build.target.is_android() {
        let android_ndk_home = std::env::var("ANDROID_NDK_HOME").unwrap();
        Config::new(&build.kwui_source_dir)
            ...
            .define("CMAKE_SHARED_LINKER_FLAGS", "-Wl,-Bsymbolic")
...
pub fn _build_old(
    ...
    cmake_command: Option<PathBuf>,
    offline: bool,
) {
    let python = &prerequisites::_locate_python3_cmd();
    ...
    _configure_skia(build, config, python, cmake_command.as_deref());
    _build_skia(config, &ninja);
}

/// Configures Skia by calling gn
pub fn _configure_skia(
    build: &FinalBuildConfiguration,
    config: &binaries_config::BinariesConfiguration,
    python: &Path,
    ...
    let gn_command = gn_command
        .map(|p| p.to_owned())
        .unwrap_or_else(|| build.kwui_source_dir.join("bin").join("gn"));
    ...
    let output = Command::new(gn_command)
        ...
        .envs(env::vars())
        .current_dir(&build.kwui_source_dir)
        .stdout(Stdio::inherit())
        .stderr(Stdio::inherit())
        .output()
        .expect("gn error");
...
pub fn _build_skia(config: &binaries_config::BinariesConfiguration, ninja_command: &Path) {
...
mod prerequisites {
    ...
    fn _is_python_version_3(exe: impl AsRef<str>) -> Option<bool> {
...
mod ninja {
    ...
    pub fn _default_exe_name() -> PathBuf {
